...
class OutputFromMultipleDevices():
    ...
    fireware = parser.parse_args().firmware
    ...
    def login_and_collect_outputs(self, devicedata, username, password, secret, commandset):
        ...
        devicedata['device_type'] = self.firmware if devicedata['device_type'] is None else devicedata['device_type']
        ...
        try:
            ...
            device = netmiko.ConnectHandler(**devicedata, username =)
            device.enable()
        except:
    ...
    def save_output(self, logindata, result):
        ...
        for devicedata, resulttemp in data:
